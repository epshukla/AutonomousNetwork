...
export default function DeviceCPUChart({ data, onDeviceClick }: Props) {
  // Merge all device timeseries into a single array keyed by time
  const deviceIds = Object.keys(data);
  if (deviceIds.length === 0) {
    return (
      <div className="glass-card p-3">
        <div className="flex items-center gap-2 mb-2">
          <Cpu className="w-3.5 h-3.5 text-noc-cyan" />
          <span className="text-xs font-bold text-noc-muted uppercase tracking-wider">CPU Per Device</span>
        </div>
  ...
  // Build merged dataset: [{time, core-delhi-1, core-delhi-2, ...}]
  ...
  for (const [deviceId, points] of Object.entries(data)) {
    for (const pt of points) {
      ...
      (entry as any)[deviceId] = pt.cpu_utilization;
    }
  }
    ...
    <div className="glass-card p-3">
      ...
      <ResponsiveContainer width="100%" height={200}>
        <AreaChart data={merged} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
          <defs>
            {deviceIds.map((id) => (
              <linearGradient key={id} id={`cpu-grad-${id}`} x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor={DEVICE_COLORS[id] || '#00d4ff'} stopOpacity={0.3} />
                <stop offset="95%" stopColor={DEVICE_COLORS[id] || '#00d4ff'} stopOpacity={0} />
              </linearGradient>
            ))}
          </defs>
          ...
          {deviceIds.map((id) => (
            <Area
              ...
              type="monotone"
              dataKey={id}
              stroke={DEVICE_COLORS[id] || '#00d4ff'}
              fill={`url(#cpu-grad-${id})`}
              strokeWidth={2}
              dot={false}
              connectNulls
            />
        ...
        {deviceIds.map((id) => (
          ...
          >
            <span
              ...
              style={{ backgroundColor: DEVICE_COLORS[id] || '#00d4ff' }}
            />
            {FRIENDLY[id] || id}
          </button>
        ))}
